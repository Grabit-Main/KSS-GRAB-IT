...
export default function HomePage() {
  ...
  const { popularProductsList, isPersonalizedBySearch, userTopTerms } = useMemo(() => {
    ...
    if (searchedTerms.length === 0) {
      const defaultPopular = products.filter(p => ['snacks','dairy','beverages','staples','household', '1', '2', '3', '4', '7'].includes(String(p.category || p.category_slug))).slice(0, isMobile ? 6 : 12);
    ...
    const remaining = products.filter(p => !matchedIds.has(p.id) && ['snacks','dairy','beverages','staples','household', '1', '2', '3', '4', '7'].includes(String(p.category || p.category_slug)));
  ...
  // Responsive grid columns & max 2 lines of categories
  ...
  return (
    <div className="professional-design-background" style={{ minHeight: '100vh', paddingBottom: '48px', position: 'relative' }}>
      ...
      <div id="categories-grid" className="container" style={{ paddingTop: isMobile ? '12px' : '16px', marginBottom: isMobile ? '20px' : '32px', position: 'relative', zIndex: 2 }}>
        <div className="professional-card" style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: catCols, gap: isMobile ? '8px' : '14px' }}>
            {categories.slice(0, maxCatCount).map((cat, idx) => {
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: isMobile ? '14px' : '18px', flexWrap: 'wrap', gap: '8px' }}>
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <h2 style={{ fontSize: isMobile ? '16px' : '20px', fontWeight: 900, color: '#1D1D1F', margin: 0 }}>
                  Popular Near You
                </h2>
      ...
      {/* ── 4.5. MIDNIGHT CRAVINGS & PARTY SNACKS HERO BANNER ────────────────────── */}
      {isMobile ? (
        <div className="container" style={{ paddingTop: '4px', marginBottom: '16px', position: 'relative', zIndex: 2 }}>
      ...
      {/* ── 6.5. EXCLUSIVE DEALS & OFFERS PURPLE HERO BANNER ────────────────────── */}
      <div className="container" style={{
        ...
        <Link to="/exclusive-deals" style={{ textDecoration: 'none', width: '100%', maxWidth: isMobile ? '100%' : '980px' }}>
          <div style={{
      ...
      {/* ── 9. PIXEL-PERFECT VIP SAVINGS & COUPON CLAIM ─────────────────── */}
      <div className="container" style={{ paddingTop: isMobile ? '16px' : '32px', paddingBottom: isMobile ? '85px' : '0px' }}>
        <div style={{
          background: 'linear-gradient(135deg, #F0F7FF 0%, #E0F0FE 50%, #EFF6FF 100%)',
          ...
          <div style={{
            ...
            <div style={{
              ...
              <img
                src="/vip-gift-box-3d.png?v=transparent_large"
                alt="VIP Coupon 3D Gift Box"
                style={{
                  width: isMobile ? '170px' : '280px',
                  height: 'auto',
                  objectFit: 'contain',
                  filter: 'drop-shadow(0 14px 32px rgba(0,113,227,0.25))'
                }}
              />
            </div>

            {/* Content & Input Form in Middle/Right */}
            <div style={{ flex: 1, minWidth: 0, width: '100%' }}>
              ...
              <h2 style={{
                ...
                Get Flat <span style={{ color: '#0071E3' }}>₹100 OFF</span> Coupon
              </h2>
              ...
              {/* Email Claim Form Bar */}
              <form
                onSubmit={(e) => {
                  e.preventDefault();
                  alert('🎉 Congratulations! ₹100 Coupon Code GRABIT100 sent to your email!');
                }}
                style={{
                  display: 'flex',
                  background: '#FFFFFF',
                  ...
                  boxShadow: '0 6px 20px rgba(0, 113, 227, 0.18)',
                  maxWidth: '460px',
                  alignItems: 'center'
                }}
              >
